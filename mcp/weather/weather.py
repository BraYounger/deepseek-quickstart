...
def format_datetime(iso_string: str) -> str:
    """
    将 ISO 8601 格式的日期时间字符串转换为 YYYY-MM-DD HH:MM 格式
    
    参数:
        iso_string: 输入的日期时间字符串，如 "2025-07-09T06:00:00-07:00"
    
    返回:
        格式化后的日期时间字符串 (如 "2025-07-09 06:00")，
        如果输入格式无效则返回原字符串
    """
    try:
...
@mcp.tool()
async def get_forecast(latitude: float, longitude: float, query_date_str: str) -> str:
    """
    根据给定的经纬度获取天气预报。
    ...
    for period in periods:
        if not period['startTime'].startswith(query_date_str):
            continue
        forecast = f"""
{format_datetime(period['startTime'])} - {format_datetime(period['endTime'])}:
